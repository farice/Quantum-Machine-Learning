...
\begin{align*}
\< f, g \> &= E_{x \sim \{-1, +1\}^n}[f(x)g(x)] = \sum_{S \in [n]} \hat{f}(S) \hat{g}(S)	 \tag{Plancherel's Theorem}
\end{align*}
...
\begin{proof}
	First, we know that $\hat{f}(S) = \< f(x), \chi_S(x) \>$ (recall that this is an expectation across all $x \in \{-1, +1\}^n$) from the previous section. Hence, given $(x_1, f(x_1))$, we can compute $f(x_1)\chi_S(x_1) \in \{-1, +1\}$ to empirically estimate $\hat{f}(S)$. From here, we simply apply a Chernoff bound (Corollary \ref{cor:chernoff-bound}) to determine the number of samples required to satisfy our claim.
	
	Say we are given random samples $x_1, \cdots, x_s$. Then, write random variable $X_i = f(x_i)\chi_S(x_i)$
