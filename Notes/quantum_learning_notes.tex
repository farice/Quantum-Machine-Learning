...
and output random row $Y = A_{(i, \cdot)} / \sqrt{s p_i} = \frac{1}{\sqrt{s}}\frac{A_{(i, \cdot)}}{\Vert A_{(i, \cdot)}\Vert}\Vert A\Vert_F$ (which is just a scaling of the $i$th row of $A$\footnote{The reason that we scale by $s$ is so that the expectations of $A^\dag A$ and $R^\dag R$ coincide in the theorem that follows. The reason that we scale by $p_i$ is so that the norms of all rows are equivalent---a fact which we'll utilize when we sample $R$ column-wise.}). In other words,
...
\begin{align*}
	\Vert R^\dag R - A^\dag A \Vert \epsilon\leq \Vert A \Vert \Vert A \Vert_F
\end{align*}
