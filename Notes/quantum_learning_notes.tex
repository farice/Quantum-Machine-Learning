...
\end{lemma}
\begin{proof}
Say that we have the ensemble $\{ (1/2, \rho_1), (1/2, \rho_2)\}$ where we chose $1/2$ because it gives the worst case where each density matrix is equally likely. We seek to define a POVM $\{E_1, E_2 \}$ where $E_1$ indicates $\rho_1$ and similarly for $\rho_2$. hence the probability of success is given by

\begin{align*}
1/2\Tr(E_1\rho_1) + 1/2\Tr(E_2\rho_2) \\
&= 1/2\Tr(E_1\rho_1) + 1/2\Tr((I - E_1)\rho_2) \tag{completeness of POVM} \\
&= 1/2\Tr(\rho_2) + 1/2\Tr(\rho_1 - \rho_2) \tag{linearity of trace} \\
&= 1/2[ 1 + \Tr(\rho_1 - \rho_2)] \tag{$\Tr(\rho) = 1$ for any density operator}
\end{align*}
