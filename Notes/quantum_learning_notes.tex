...
and draw a fixed random element $V \in SU(4)$. Then, generate data by drawing random $x \in (0, 2\pi]^2$ and labelling based on decision rule

\begin{align*}
f(x) = \bra{x} \mathcal{U}_{\Phi(x)}^\dag V^\dag Z_1 Z_2 V \mathcal{U}_{\Phi(x)}\ket{x}	 
\end{align*}
...
\begin{align*}
\tilde{f}(x) &= \bra{x} \mathcal{U}_{\Phi(x)}^\dag W(\theta)^\dag Z_1 Z_2 W(\theta) \mathcal{U}_{\Phi(x)}\ket{x})
\end{align*}
