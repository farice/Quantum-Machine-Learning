...
As described above, we begin by length-square sampling the original matrix $A \in \CC^{m \times n }$. Suppose we want to draw $r$ columns in $r$ i.i.d. trials. Then, pick row index $i$ with probability

\begin{align*}
p_i = \frac{\Vert A_{(i, \cdot)}\Vert^2}{\Vert A \Vert_F^2} 	
\end{align*}

and output random row $Y = A_{(i, \cdot)} / \sqrt{s p_i} = \frac{1}{\sqrt{s}}\frac{A_{(i, \cdot)}}{\Vert A_{(i, \cdot)}\Vert}\Vert A\Vert_F$ (which is just a scaling of the $i$th row of $A$). In other words,

\begin{align*}
\Pr(Y = A_{(i, \cdot)} / \sqrt{s p_i}) = p_i
\end{align*}


After sampling $r$ rows, we implicitly define matrix $R$ to be the concatenation of the outputted random rows. Furthermore, we rescale $R$ so that $E[R^{\dag} R] = A^\dag A$, which requires normalizing by $\sqrt{r}$ (shown in Theorem ). Therefore,
...
Note that $Y_k$ denotes the random row outputted by the procedure on the $k$th i.i.d. draw.

\begin{theorem}
Let $A \in \CC^{m \times n}$ and $R \in \CC^{r \times n}$ be constructed by the length-square sampling process described above. 	
\end{theorem}
