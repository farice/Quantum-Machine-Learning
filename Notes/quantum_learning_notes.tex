...
The domain of a boolean function is the "Hamming cube". 

\begin{definition}
Let $S \subseteq [n], n \in \ZZ$. Then,
\begin{align*}
\chi_S(x) &:= \sum_{i\in S} x_i \tag{with $x^{\emptyset} = 1$ by convention}
\end{align*}	
\end{definition}
...
\begin{proposition}
	The set of all functions $f: \{-1, +1\}^n \rightarrow \RR$ forms a vector space $V$ s.t. $dim V = 2^n$. The $\{\chi_S(x)\}, S \subseteq [n]$ form an orthonormal basis for $V$. 
\end{proposition}
