...
From above, $p(m) = \bra{\psi} M_m^\dagger M_m \ket{\psi}$ so if we define $E_m := M_m^\dagger M_m$ then these $E_m$'sd are sufficient for the purpose of computing probabilities. We denote $\{ E_m \}$ as a POVM. 
...
From this exercise, we see that POVMs present a reliable way to distinguish non-orthogonal states given that we allow for the slack of an "inconclusive" measurement ($E_{m+1}$). 
...
We can show that $U$ can be extended to a unitary operator on $Q \otimes M$ (exercise).

Furthermore, consider the two systems given by projectors $P_m := I_Q \otimes \ket{m}\bra{m}$.
